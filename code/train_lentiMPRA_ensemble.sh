...
if [ "$downsample" = true ]; then
    ...
    do
        ...
        done | simple_gpu_scheduler --gpus 3,4,5,6
    done 
else
    for i in $(seq 1 $ENSEMBLE_SIZE)
    do 
        if [ "$evoaug" = true ]; then
            echo "python train_lentiMPRA.py --ix $i --out $OUTDIR --data $DATA --plot --config $CONFIG --project $PROJECT_NAME --lr_decay --evoaug --celltype $CELLTYPE"
        else
    ...
    done | simple_gpu_scheduler --gpus 5,6,7
fi
