...
# class DynamicAuglentiMPRA(keras.Model):
#     '''
#     Tensorflow keras.Model class lentiMPRA model (ResidualBind) with dynamic sequence augmentations 
...
#         function defining a keras Model
#     ensemble : list
#         list of keras Models comprising the ensemble; models predict activity and aleatoric uncertainty
#     input_shape : tuple, list
#         shape of model input (excluding batch dim)
#     augment_list : list
#         list of evoaug augmentations, each a callable class from evoaug_tf.augment; default=[]
...
#     def __init__(self, model_func, ensemble=None, input_shape=None, aug=None, augment_list=[],
#                 max_augs_per_seq=2, inference_aug=False, hard_aug=False, append=False, **kwargs):
#         super(DynamicAuglentiMPRA, self).__init__()
#         self.model = model_func
#         self.append = append
#         self.ensemble = ensemble
#         self.aug = aug
...
#         all_preds = tf.stack(all_preds)
#         if self.kwargs['epistemic']:
#             # ensemble should predict mean of activity and aleatoric uncertainty predictions 
#             # need to get ensemble std of activity preds (first column) for epistemic uncertainty 
#             return tf.concat([tf.math.reduce_mean(all_preds, axis=0), tf.math.reduce_std(all_preds[:,0], axis=0)], axis=1)
#         else: 
#             return tf.math.reduce_mean(all_preds,axis=0)
