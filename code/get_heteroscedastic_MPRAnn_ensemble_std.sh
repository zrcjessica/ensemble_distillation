...
if [ "$DOWNSAMPLE" = true ]; then
	...
	do	
		if [ "$EVOAUG" = true ]; then
			CUDA_VISIBLE_DEVICES=7 python get_heteroscedastic_MPRAnn_ensemble_std.py --model_dir $MODELS_DIR --n_mods $N_MODS --data $DATA --out $OUTDIR --evoaug --config $MODELS_DIR/config.yaml --downsample ${DOWNSAMPLE_ARR[$p]} --celltype $CELLTYPE 
		else
			CUDA_VISIBLE_DEVICES=7 python get_heteroscedastic_MPRAnn_ensemble_std.py --model_dir $MODELS_DIR --n_mods $N_MODS --data $DATA --out $OUTDIR --downsample ${DOWNSAMPLE_ARR[$p]} --celltype $CELLTYPE 
		fi
	done
else
	if [ "$EVOAUG" = true ]; then
		CUDA_VISIBLE_DEVICES=7 python get_heteroscedastic_MPRAnn_ensemble_std.py --model_dir $MODELS_DIR --n_mods $N_MODS --data $DATA --out $OUTDIR --evoaug --config $MODELS_DIR/config.yaml --celltype $CELLTYPE 
	else
		CUDA_VISIBLE_DEVICES=7 python get_heteroscedastic_MPRAnn_ensemble_std.py --model_dir $MODELS_DIR --n_mods $N_MODS --data $DATA --out $OUTDIR --celltype $CELLTYPE 
	fi
fi 
