...
code is currently designed for analyzing attribution scores for top 1000 lentiMPRA test seqs
'''
...
def parse_args():
    ...
    parser.add_argument('--celltype', type=str, 
                        help='celltype that lentiMPRA model corresponds to')
...
def main(args):
    ...
    # get files
    attr_files, ref_file = utils.get_lentiMPRA_attribution_files(args.files_dir, args.method, args.celltype, args.top_n, args.reference)
    ...
    if args.rmse:
        ...
        for f in attr_files:
            ...
            # write to file 
            np.save(join(outdir, f"{model_ix}_top{args.top_n}_{args.celltype}_{args.method}_euclidean_dist.npy"), distances)
    ...
    if args.var:
        ...
        np.save(join(outdir, f"top{args.top_n}_{args.celltype}_{args.method}_stdev.npy"), seq_std)
