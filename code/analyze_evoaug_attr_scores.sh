# analyzes attribution scores (shap/saliency) for a set of DeepSTARR models trained w/ EvoAug
...
if [ "$DOWNSAMPLED" = true ]; then
	echo "Analyzing $METHOD scores for DeepSTARR models (+EvoAug) trained on subset of training data"
...
if command -v 'slack' &>/dev/null; then
    if [ "$exit_code" -eq 0 ]; then
		if [ "$DOWNSAMPLED" = true ]; then
            if [ "$DISTILLED" = true ]; then
                slack "running analyze_attr_scores.py on DeepSTARR (+EvoAug) (downsampled & distilled) $METHOD scores completed successfully" &>/dev/null 
            else 
                slack "running analyze_attr_scores.py on DeepSTARR (+EvoAug) (downsampled & not distilled) $METHOD scores completed successfully" &>/dev/null
            fi 
        else
            if [ "$DISTILLED" = true ]; then 
                slack "running analyze_attr_scores.py on DeepSTARR (+EvoAug) (trained on full dataset & distilled) $METHOD scores completed successfully" &>/dev/null 
            else 
                slack "running analyze_attr_scores.py on DeepSTARR (+EvoAug) (trained on full dataset & not distilled) $METHOD scores completed successfully" &>/dev/null 
            fi 
        fi 
    else 
        if [ "$DOWNSAMPLED" = true ]; then
