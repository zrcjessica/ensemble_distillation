...
evoaug=true
if [ "$evoaug" = true ]; then
    OUTDIR_ROOT=../results/DeepSTARR_evoaug
...
if command -v 'slack' &>/dev/null; then
    if [ "$exit_code" -eq 0 ]; then
		slack "training downsampled ensembles of $ENSEMBLE_SIZE DeepSTARR models w/ EvoAug (p=0.5) completed successfully" &>/dev/null
	else
