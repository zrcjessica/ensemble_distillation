...
'''
train an ensemble of MPRAnn models w/ MPRAnn
...
This script is used to train MPRAnn models w/ predictions for activity and aleatoric. 
Another script will be used to distill MPRAnn models w/ mean+aleatoric+epistemic predictions.
'''
...
def eval_performance(model, X_test, y_test, outfh, celltype, aleatoric=False, epistemic=False):
    ...
    if y_pred.shape != y_test.shape:
        results = utils.summarise_MPRAnn_performance(y_pred, np.expand_dims(y_test, axis=-1), celltype, aleatoric=aleatoric, epistemic=epistemic)
    else:
        results = utils.summarise_MPRAnn_performance(y_pred, y_test, celltype, aleatoric=aleatoric, epistemic=epistemic)
...
def main(args):
    ...
    if args.evoaug:
        ...
        # training w/o evoaug
        model = MPRAnn(X_train[0].shape, aleatoric=args.aleatoric, epistemic=False)
    ...
    if args.evoaug:
        ...
        if args.evidential:
            # save model weights
            model.save_weights(join(args.out, f'{args.ix}_MPRAnn.weights.h5'))
        else:
