...
if command -v 'slack' &>/dev/null; then
    if [ "$exit_code" -eq 0 ]; then
		slack "evaluating performance of distilled DeepSTARR model with standard deviation prediction completed successfully" &>/dev/null
	else
		slack "evaluating performance of distilled DeepSTARR model with standard deviation prediction  exited with error code $exit_code"
	fi
fi
exit "$exit_code"