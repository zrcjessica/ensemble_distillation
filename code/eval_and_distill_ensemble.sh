...
MODEL_DIR=../results/DeepSTARR_ensemble_NEW
N_MODS=10
...
if [ "$DOWNSAMPLED" = true ]; then
	...
	do
		...
		CUDA_VISIBLE_DEVICES=7 python ensemble_predict_DeepSTARR.py --model_dir ${MODEL_DIR}/downsample_${DOWNSAMPLE_ARR[$p]} --n_mods $N_MODS --data $DATA --distill --eval --downsample ${DOWNSAMPLE_ARR[$p]} 
	done 
else
...
if command -v 'slack' &>/dev/null; then
    if [ "$exit_code" -eq 0 ]; then
		if [ "$DOWNSAMPLED" = true ]; then
			...
			slack "running ensemble_predict_DeepSTARR.py for $MODEL_DIR (full) in distill and eval mode completed successfully" &>/dev/null
		fi
	else
		if [ "$DOWNSAMPLED" = true ]; then
			slack "running ensemble_predict_DeepSTARR.py for $MODEL_DIR (downsampled) in distill and eval mode exited with error code $exit_code"
		else
			slack "running ensemble_predict_DeepSTARR.py for $MODEL_DIR (full) in distill and eval mode exited with error code $exit_code"
		fi
	fi
fi
exit "$exit_code"
