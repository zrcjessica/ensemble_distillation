...
if command -v 'slack' &>/dev/null; then
    if [ "$exit_code" -eq 0 ]; then
		slack "performing $METHOD analysis for replicate DeepSTARR models (no std. prediction) for binned sequences completed successfully" &>/dev/null
	else
		slack "performing $METHOD analysis for replicate DeepSTARR models (no std. prediction) for binned sequences exited with error code $exit_code"
	fi
fi
exit "$exit_code"