...
'''
train an ensemble of MPRAnn models on lentiMPRA data using heteroscedastic regression
...
This script is used to train MPRAnn models w/ predictions for activity and aleatoric (derived from heteroscedastic regression). 
'''
...
def main(args):
    ...
    if args.evoaug:
        ...
        model = evoaug.RobustModel(MPRAnn_heteroscedastic, 
        ...
        # training w/o evoaug
        model = MPRAnn_heteroscedastic(X_train[0].shape)
    ...
    if args.evoaug:
        # save weights
        save_path = join(args.out, f"{args.ix}_MPRAnn_heteroscedastic_aug_weights.h5")
        ...
        model = evoaug.RobustModel(MPRAnn_heteroscedastic, input_shape=X_train[0].shape, 
                                   augment_list=augment_list, 
        ...
        model.save_weights(join(args.out, f"{args.ix}_MPRAnn_heteroscedastic_finetune.h5"))
        ...
        # save model and history
        model.save_weights(join(args.out, f"{args.ix}_MPRAnn_heteroscedastic.h5"))
