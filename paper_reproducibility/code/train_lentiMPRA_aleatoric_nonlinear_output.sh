...
if [ "$downsample" = true ]; then
    ...
    do
        ...
        do 
            if [ "$evoaug" = true ]; then
                echo "echo 'model_ix=$i' && python train_lentiMPRA_v2.py --ix $i --out $OUTDIR_DOWNSAMPLE --data $DATA --plot --downsample ${DOWNSAMPLE_ARR[$p]} --config $CONFIG --project $PROJECT_NAME --lr_decay --evoaug --celltype $CELLTYPE --aleatoric"
            else
                echo "python train_lentiMPRA_v2.py --ix $i --out $OUTDIR_DOWNSAMPLE --data $DATA --plot --downsample ${DOWNSAMPLE_ARR[$p]} --config $CONFIG --project $PROJECT_NAME --lr_decay --celltype $CELLTYPE --aleatoric"
            fi
    ...
    do 
        if [ "$evoaug" = true ]; then
            echo "python train_lentiMPRA_v2.py --ix $i --out $OUTDIR --data $DATA --plot --config $CONFIG --project $PROJECT_NAME --lr_decay --evoaug --celltype $CELLTYPE --aleatoric"
        else
            echo "python train_lentiMPRA_v2.py --ix $i --out $OUTDIR --data $DATA --plot --config $CONFIG --project $PROJECT_NAME --lr_decay --celltype $CELLTYPE --aleatoric"
        fi
