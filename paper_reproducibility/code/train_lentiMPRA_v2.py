...
import utils
from model_zoo import lentiMPRA_v2
import plotting
import pandas as pd
import wandb
...
'''
train an ensemble of lentiMPRA models w/ separate and nonlinear output heads 
...
def main(args):
    ...
    if args.evoaug:
        ...
        model = evoaug.RobustModel(lentiMPRA_v2, 
        ...
        # training w/o evoaug
        model = lentiMPRA_v2(X_train[0].shape, wandb.config, aleatoric=args.aleatoric, epistemic=args.epistemic)
    ...
    if args.evoaug:
        ...
        model = evoaug.RobustModel(lentiMPRA_v2, input_shape=X_train[0].shape, augment_list=augment_list, max_augs_per_seq=2, hard_aug=True, config=wandb.config, aleatoric=args.aleatoric, epistemic=args.epistemic)
        model.compile(optimizer=Adam(learning_rate=args.lr), loss=wandb.config['loss_fxn'])
